...
namespace CollegeExamFreeReport
{
    public class Program
    {
        ...
        {   // [ischoolkingdom] Vicky更動，原107年五專(優先)免試入學相關報表，移除"107年"字樣
            ...
            item1["報表"]["成績相關報表"]["五專(優先)免試入學相關報表"]["超額比序項目積分證明單(一般免試入學)"].Enable = false;
            item1["報表"]["成績相關報表"]["五專(優先)免試入學相關報表"]["超額比序項目積分證明單(一般免試入學)"].Click += delegate
            {
            ...
            K12.Presentation.NLDPanels.Student.SelectedSourceChanged += delegate
            {
                ...
                {
                    item1["報表"]["成績相關報表"]["五專(優先)免試入學相關報表"]["超額比序項目積分證明單(一般免試入學)"].Enable = true;
                }
                else
                {
                    item1["報表"]["成績相關報表"]["五專(優先)免試入學相關報表"]["超額比序項目積分證明單(一般免試入學)"].Enable = false;
